...
const AlterLedgerMaster = () => {
  ...
  const loadLedger = async () => {
    try {
      const result = await axios.get(`http://localhost:9080/ledgerMasterApi/displayLedger/${ledgerCode}`);
    ...
    <div>
      <div className='flex'>
        ...
        <div className='w-1/2 border h-[100vh]'>
          <div className='w-[550px] h-[30px] flex justify-between text-[20px] bg-[#F1E5D1] ml-[750px] mt-10 border border-gray-500 border-b-0'>
            ...
            <span className='cursor-pointer mt-[5px] mr-2'>
              <Link to={"/ledgerAlter"}><IoClose /></Link>
            </span>
          </div>
          ...
          <div className='w-[550px] h-[10vh] border border-gray-500 ml-[750px]'>
            <form onSubmit={onSubmit}>
              ...
              <div className='mt-[440px]'>
                ...
                <input type="button" id='acceptButton' onKeyDown={(e) => {if(e.key === 'Backspace'){e.preventDefault(); if(inputRefs.current.ledgerName && inputRefs.current.ledgerName.focus){inputRefs.current.ledgerName.focus(); }}}} value={"A: Accept"} ref={(button) => {acceptButtonRef.current = button; }} onClick={(e) => onSubmit(e)} className='text-sm px-8 py-1 mt-3 border bg-slate-600 hover:bg-slate-800 ml-[100px]' />
              </div>
        ...
        <div className='mt-[450px] ml-[1150px]'>
          <Link to={"/ledgerAlter"} className='border px-11 py-[5px] text-sm bg-slate-600 hover:bg-slate-800 '>Q:Quit</Link>
        </div>
      </div>
    </div>
...
export default AlterLedgerMaster;
