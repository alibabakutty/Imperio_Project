...
const AlterVoucherTypeMaster = () => {
    ...
    if (value.trim() !== "") {
      ...
      if (exactMatch) {
        setVoucher((prevState) => ({
          ...prevState,
          voucherType: exactMatch.voucherType,
        }));
      }
    } else {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    await axios.put(
      `http://localhost:9080/voucherTypeMasterApi/alterVoucherTypeMaster/${voucherTypeName}`,
      voucher
    );
    navigate('/voucherTypeAlter/filter');
  };
  ...
  const handleSubFormCancel = () => {
    setVoucher({
      ...voucher,
      startingNumber: voucher.startingNumber,
      widthOfNumericalPart: voucher.widthOfNumericalPart,
      prefillWithZero: voucher.widthOfNumericalPart,
      restartNumberingApplicationForm: voucher.restartNumberingApplicationForm,
      restartNumberingStartingNumber: voucher.restartNumberingStartingNumber,
      restartNumberingPeriodicity: voucher.restartNumberingPeriodicity,
      prefixDetailsApplicationForm: voucher.prefixDetailsApplicationForm,
      prefixDetailsParticulars: voucher.prefixDetailsParticulars,
      suffixDetailsApplicationForm: voucher.suffixDetailsApplicationForm,
      suffixDetailsParticulars: voucher.suffixDetailsParticulars,
      alterAdditionalNumberingDetails: voucher.alterAdditionalNumberingDetails,
    });
  ...
  const voucherNumberRenderingOptions = () => {
    ...
    return methodsOfNumberingDatas.map((methodOfNumbering) => (
      <p
        key={methodOfNumbering}
        tabIndex={0}
        onClick={() => handleVoucherNumberingClick(methodOfNumbering)}
        onKeyDown={(e) => {
          if (e.key === "Enter") {
            handleVoucherNumberingClick(methodOfNumbering);
          }
        }}
        className="h-5 pl-2 text-sm focus:bg-yellow-200 focus:outline-none text-left"
      >
        {methodOfNumbering}
      </p>
    ));
  ...
  const handleDateInputChange = (e) => {
    ...
    switch (name) {
      case "restartNumberingApplicationForm":
        setVoucher({
          ...voucher,
          restartNumberingApplicationForm: formattedValue,
        });
        break;
      case "prefixDetailsApplicationForm":
        setVoucher({
          ...voucher,
          prefixDetailsApplicationForm: formattedValue,
        });
        break;
      case "suffixDetailsApplicationForm":
        setVoucher({
          ...voucher,
          suffixDetailsApplicationForm: formattedValue,
        });
        break;
    }
  };
    ...
    <div>
      <div className="#FFF5E1 w-[90%] h-[95vh]">
        <div className="text-[12px] flex justify-between bg-[#80C4E9]">
          ...
          <span className="mt-1">
            <Link to={"/voucherTypeAlter/filter"}>
              <IoClose />
            </Link>
          </span>
        </div>
                        ...
                        {filteredVoucherTypeSuggestions.map(
                          (voucher, index) => (
                            <li
                              ...
                              onKeyDown={(e) =>
                                e.key === "Enter" && selectVoucherType(voucher)
                              }
                ...
                <div>
                  ...
                  {showVoucherNumberingOptions && (
                    <div
                      ...
                      className="w-[15%] border text-center text-sm bg-[#CAF4FF] absolute top-[122px] left-[405px]"
                    >
                                  ...
                                  <div className="flex justify-evenly">
                                    ...
                                    <div>
                                      ...
                                      {showPeriodicityOptions && (
                                        <div
                                          ref={optionsRef}
                                          className="w-[10%] border text-sm bg-[#CAF4FF] absolute top-[92px] left-[350px]"
                                        >
                                          <div className="bg-[#003285] px-5 text-white">
                                            <p>Periodicity</p>
                                          </div>
        ...
        <div>
          <div className="ml-[480px]">
            ...
            />
            <span className="text-sm absolute top-[583px] left-[503px] underline decoration-black" style={{textDecorationThickness: '2px'}}>A</span>

            <Link
              to={"/voucherTypeAlter/filter"}
              className="border px-11 py-[5px] text-sm bg-slate-600 hover:bg-slate-800 ml-[250px]"
            >
