...
const RegionFilter = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      const totalItems =
        region.length > 20
          ? filteredRegions.length + 3
          : filteredRegions.length + 2; // +2 for Create, Back, and +1 for dropdown if it exists
      ...
      if (e.key === "ArrowDown") {
        ...
        if (selectedIndex === 0) {
          ...
          navigate(
            `/displayRegion/${
              filteredRegions[selectedIndex - 2].regionMasterId
            }`
                  ...
                  {filteredRegions.map((reg, index) => (
                    ...
                    >
                      <td className="block text-left text-[12.5px] capitalize pl-2">
                        <Link
                          to={`/displayRegion/${reg.regionMasterId}`}
                          className="block"
                        >
