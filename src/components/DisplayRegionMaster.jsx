    ...
    const handleKeyDown = (event) => {
        ...
        if (keyCode === 13) { // Enter key
          ...
          if (currentInputIndex === Object.keys(inputRefs.current).length - 2) { // Check if it's the last input field
            backButtonRef.current.focus(); // Focus on the accept button
          } else {
        ...
        <div>
            <div className='flex'>
                ...
                <div className='w-1/2 border border-bg-gray-500'>
                    ...
                    <Link to={"/regionFilter"} id='backButton' ref={(button) => {backButtonRef.current = button; inputRefs.current.backButton = button}} className='border px-11 py-[5px] text-sm bg-slate-600 hover:bg-slate-800 '>Back</Link>
                </div>
