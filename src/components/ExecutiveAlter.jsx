        ...
        const handleKeyDown = (e) => {
            if (e.key === 'ArrowDown') {
                ...
                if (selectedIndex === 0) {
                    ...
                    navigate(`/alterExecutiveMaster/${filteredExecutives[selectedIndex - 2].executiveCode}`);
                }
            }
        };
        ...
        <div className='flex'>
            ...
            <div className='w-[45%] h-[100vh] bg-[#EEEEEE] flex flex-col items-center justify-start'>
                ...
                <div className='w-[350px] h-[85vh] border border-gray-600 bg-[#def1fc]'>
                    <h2 className='p-1 bg-[#2a67b1] text-white text-center text-[14px]'>List of Executives</h2>
                    <table>
                        <thead>
                            <tr>
                                <th></th>
                        ...
                        <div className='border border-b-gray-500 w-[347px]'>
                            <Link className={`block text-center text-[14px] focus:bg-[#FEB941] outline-none ${selectedIndex === 0 ? 'bg-[#FEB941]' : ''}`} to={"/executive"}>
                                <p className='ml-[285px] text-[14px]'>Create</p>
                            </Link>
                            <Link className={`block text-center text-[14px] focus:bg-[#FEB941] outline-none ${selectedIndex === 1 ? 'bg-[#FEB941]' : ''}`} to={"/alter"}>
                            ...
                            {filteredExecutives.map((exe, index) => (
                                <tr key={exe.executiveCode} className={selectedIndex === index + 2 ? 'bg-[#FEB941]' : ''}>
                                    <td className='flex justify-center items-center capitalize'>
                                        <Link className='block' to={`/alterExecutiveMaster/${exe.executiveCode}`}>
                                            {exe.executiveCode}
                                        </Link>
                                    </td>
                                </tr>
