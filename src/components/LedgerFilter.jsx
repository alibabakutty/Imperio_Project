    ...
    const handleKeyDown = (e) => {
      if(e.key === 'ArrowDown'){
        ...
        if(selectedIndex === 0){
          ...
          navigate(`/displayLedger/${filteredLedgers[selectedIndex - 2].ledgerCode}`);
        }
      }
    };
  ...
  return (
    ...
    <div className='flex justify-evenly'>
        ...
        <div className='w-[90%] flex h-screen'>
          ...
          <p className='text-[13px] font-semibold underline underline-offset-4 decoration-gray-400'>Ledger Display</p>
        ...
        <div className='w-[350px] h-[85vh] border border-gray-600 bg-[#def1fc]'>
          <h2 className='p-1 bg-[#2a67b1] text-white text-left text-[13px]'>List of Ledgers</h2>
          <table>
              <thead>
                  <tr>
                      <th></th>
              ...
              <div className='border border-b-gray-500 w-[347px]'>
                  <Link className={`block text-center text-[13px] focus:bg-[#FEB941] outline-none ${selectedIndex === 0 ? 'bg-[#FEB941]' : ''}`} to={"/ledger"}><p className='ml-[285px] text-[14px]'>Create</p></Link>
              ...
              <tbody>
                  {filteredLedgers.map((led,index) => (
                    <tr key={led.ledgerCode} className={selectedIndex === index + 2 ? 'bg-[#FEB941]': ''}>
                        <Link className='block text-center text-[13px] focus:bg-[#FEB941] outline-none' to={`displayLedger/${led.ledgerCode}`}>
                            <td className='flex text-left capitalize pl-2'>{led.ledgerCode}</td>
                        </Link>
                    </tr>
                  ))}
              </tbody>
